...
Java_co_airbitz_internal_Jni_coreSweepKey(
    ...
    result = (tABC_CC)ABC_SweepKey(
                (char const *)username, (char const *)password,
                (char const *)uuid, (char const *)wif, ret,
                callback, bcInfo, errorp);
    jresult = (jint)result;
    if (username)
        (*jenv)->ReleaseStringUTFChars(jenv, jusername, (const char *)username);
    if (password)
        (*jenv)->ReleaseStringUTFChars(jenv, jpassword, (const char *)password);
    if (uuid)
        (*jenv)->ReleaseStringUTFChars(jenv, juuid, (const char *)uuid);
    if (wif)
        (*jenv)->ReleaseStringUTFChars(jenv, jwif, (const char *)wif);
    return jresult;
}
